...
class ZaloClient {
    ...
    public function __construct() {
        $zaloConfig = new ZaloConfig();
        $this->zalo = new Zalo($zaloConfig->getConfig());
    }
    /**
     ...
     */
    public function createProduct($name,$desc,$filePaths,$price= 15000,$quantity=1,$code="",$cateids=[],$display='show',$payment= 2){
        $photos = [];
        foreach($filePaths as $filePath){      
            $photos[] = ['id'=>$this->uploadPhoto($filePath)];
        }
        $data = array(  
            'cateids' => $cateids,                
            'name' => $name,
    ...
    public function getListProduct($offset=0,$count=10){
        ...
        $params = ['data' => $data];
        $response = $this->zalo->get(ZaloEndpoint::API_OA_STORE_GET_SLICE_PRODUCT, $params);
        return $result = $response->getDecodedBody()['data']; // result
    }
    /**
     * Get Product
